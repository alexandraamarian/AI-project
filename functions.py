# -*- coding: utf-8 -*-
"""
Author: Brandstetter, Schäfl, Schörgenhumer
Date: 18-10-2021
...
def plot_points_2d(data: pd.DataFrame, target_column: Optional[str] = None, legend: bool = True, **kwargs) -> None:
    ...
    scatter_plot = ax.scatter(x=data[0], y=data[1], c=data[target_column] if target_column is not None else None)
    if (target_column is not None) and legend:
        scatter_legend = ax.legend(*scatter_plot.legend_elements(), title=target_column)
